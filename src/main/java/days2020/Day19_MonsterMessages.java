...
public class Day19_MonsterMessages {
	...
	public static void main(String[] args) {
		System.out.println("answer A: " + runA(textValidations(), textLines())); // 76 wrong.
		System.out.println("answer B: " + runB(textValidations()));
	}

	public static long runA(String validations, String input) {
		Map<String, ValidationRule> validationRuleMap = Arrays.stream(validations.split("\n"))
				.map(ValidationRule::new).collect(Collectors.toMap(v -> v.name, v -> v));
		validationRuleMap.values().forEach(v -> v.setValidations(validationRuleMap));

		ValidationRule rule0 = validationRuleMap.get("0");
		List<String> permutations = rule0.getPermutations();

		return Arrays.stream(input.split("\n"))
				.filter(s -> permutations.contains(s))
				.count();
	}
	...
	private static class ValidationRule {
		...
		String validation1;
		String validation2;
		List<ValidationRule> rule1;
		List<ValidationRule> rule2;
		char c;
		...
		public ValidationRule(String string) {
		...
		List<String> getPermutations() {
			...
			getPermutationsForRule(rule1);
			if(rule2.size() > 1) {
				getPermutationsForRule(rule2);
			}
			return permutations;
		}

		void getPermutationsForRule(List<ValidationRule> rules) {
			List<List<String>> collect = rules.stream().map(rule -> rule.getPermutations()).collect(Collectors.toList());
			if(collect.size() == 1) {
				permutations.addAll(collect.get(0));
				return;
			}
			// make permutations
			collect.get(0).stream().forEach(s1 -> { // s1= first part of string
				collect.get(1).stream().forEach(s2 -> {
					if(collect.size() > 2) {
						collect.get(2).stream().forEach(s3 -> {
...
//		String isValid(char[] input, int index) {
//			if(isFinisher) {
//				if(input[index] == c) {
...
//			}
//
//			isValidRule(input, index, rule1);
//			return "";
//		}
//
//		private String isValidRule(char[] input, int index, List<ValidationRule> rule) {
//			rule.get(0).isValidRule(input, index, rule);
//			return "";
//		}

		public void setValidations(Map<String, ValidationRule> validationRuleMap) {
			rule1 = createValidationList(validationRuleMap, validation1);
			rule2 = createValidationList(validationRuleMap, validation2);
		}

		private List<ValidationRule> createValidationList(Map<String, ValidationRule> validationRuleMap, String rule) {
			...
			Arrays.stream(rule.split(" ")).filter(s -> !"".equals(s)).forEach(s -> validationRules.add(validationRuleMap.get(s)));
			return validationRules;
		}
	}

	private static String textValidations() {
