...
public class Day1_SonarSweep {
	...
	public static int runA(String input) {
		...
		for (int i = 1; i < depths.size(); i++) {
			if(depths.get(i) - depths.get(i-1) > 0) {
				numberIncreasing++;
			}
		}
	...
	public static int runB(String input) {
		// it's the season to be merry, tralalala lalala,
		// it's also the season to write crappy unoptimized code that works.
		// tralalalala, lalala
		List<Integer> depths = Arrays.stream(input.split("\n"))
				.map(Integer::parseInt)
				.collect(Collectors.toList());
		...
		int numberIncreasing = 0;
		int previousSlidingWindow = depths.get(0) + depths.get(1) + depths.get(2);
		for (int i = 1; i < depths.size() - 2; i++) {
			int currentSlidingWindow = depths.get(i) + depths.get(i+1) + depths.get(i+2);
			if(currentSlidingWindow - previousSlidingWindow > 0) {
				numberIncreasing++;
			}
